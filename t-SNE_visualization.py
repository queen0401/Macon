...
class SequenceModel(nn.Module):
    ...
    def forward(self, x, x_emb, return_conv3=False, return_cat=False, return_fc=False):
        x = x.permute(0, 2, 1)
        x = self.pool(F.relu(self.conv1(x)))
        x = self.pool(F.relu(self.conv2(x)))
        conv3_output = self.pool(F.relu(self.conv3(x)))
        if return_conv3:
            return conv3_output        
        x = torch.flatten(conv3_output, 1)
        x = F.relu(self.fc1(x))
        if return_fc:
            return x
...
for fold, (train_idx, test_val_idx) in enumerate(kf.split(dataset), 1):
    ...
    for epoch in tqdm(range(100), desc=f"Fold {fold}"):
        ...
        def get_outputs_and_labels(model, data_loader, device, layer):
            model.eval()
            outputs = []
            labels = []
            with torch.no_grad():
                for mut0, mut1, par0, labels_batch in data_loader:
                    mut0, mut1, par0, labels_batch = mut0.to(device), mut1.to(device), par0.to(device), labels_batch.to(device)
                    mut0_emb = moco_model(mut0)
                    mut1_emb = moco_model(mut1)
                    features = torch.cat((mut0, mut1, mut0 - mut1, par0), dim=1).to(device)
                    if layer == 'conv3':
                        output = model(features, torch.cat((mut0_emb, mut1_emb, mut0_emb - mut1_emb), dim=1).to(device), return_conv3=True)
                    if layer == 'fc':
                        output = model(features, torch.cat((mut0_emb, mut1_emb, mut0_emb - mut1_emb), dim=1).to(device), return_fc=True)
                    if layer == 'cat':
                        output = model(features, torch.cat((mut0_emb, mut1_emb, mut0_emb - mut1_emb), dim=1).to(device), return_cat=True)
                    outputs.append(output.cpu().numpy())
                    labels.extend(labels_batch.cpu().numpy())
            outputs = np.concatenate(outputs, axis=0)
            return outputs, labels

        outputs_fc, labels_fc = get_outputs_and_labels(model, test_loader, device, layer='fc')
        outputs_conv3, labels_conv3 = get_outputs_and_labels(model, test_loader, device, layer='conv3')
        outputs_cat, labels_cat = get_outputs_and_labels(model, test_loader, device, layer='cat')
        
        tsne = TSNE(n_components=2, random_state=0)

        fc_embeddings = tsne.fit_transform(outputs_fc.reshape(outputs_fc.shape[0], -1))
        conv3_embeddings = tsne.fit_transform(outputs_conv3.reshape(outputs_conv3.shape[0], -1))
        cat_embeddings = tsne.fit_transform(outputs_cat.reshape(outputs_cat.shape[0], -1))
        ...
        def plot_tsne(tsne_embeddings, labels, save_name):
            ...
            for i in range(4):
                ...
                plt.scatter(tsne_embeddings[idx, 0], tsne_embeddings[idx, 1], c=colors[i], label=class_names[i], alpha=0.7, s=50)
        ...
        plot_tsne(fc_embeddings, labels_fc, save_name='tsne_plot_fc.png')
        plot_tsne(conv3_embeddings, labels_conv3, save_name='tsne_plot_conv3.png')
        plot_tsne(cat_embeddings, labels_cat, save_name='tsne_plot_cat.png')
